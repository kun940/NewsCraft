...
from sqlalchemy import Index, Integer, String, Enum, DateTime, ForeignKey, TEXT
...
# 创建索引
    ...
    avatar : Mapped [Optional [str]] = mapped_column (String(255),
            comment="头像URL",default='https://fastly.jsdelivr.net/npm/@vant/assets/cat.jpeg ')
    gender : Mapped [Optional [str]] = mapped_column (Enum('male', 'female','unknown'),
            comment="性别", default='unknown')
    bio : Mapped [Optional [str]] = mapped_column (String(500),
            comment="个人简介", default='这个人很懒，什么都没留下')
    ...
    updated_at : Mapped [datetime] = mapped_column (DateTime , default=datetime.now, onupdate=datetime.now,
            comment="更新时间")
    user_interest_vector:Mapped [Optional [str]] = mapped_column (TEXT,comment="用户兴趣向量序列化数据（AI加权计算）")
    user_interest_tags:Mapped[Optional [str]]=mapped_column(String(500),comment="用户偏好标签（AI统计生成，逗号分隔）")
    def __repr__ (self):
